...
namespace token{
  ...
  BufferPtr Transaction::ToBuffer() const{
    ...
    if(!encoder.Encode(buffer)){
      DLOG(ERROR) << "cannot encode Transaction.";
      buffer->clear();
      return buffer;
    }
    return buffer;
  ...
  bool Transaction::Encoder::Encode(const BufferPtr& buff) const{
    ...
    if(!buff->PutTimestamp(timestamp)){
      DLOG(ERROR) << "cannot encode transaction timestamp: " << FormatTimestampReadable(timestamp);
      return false;
    }
    ...
    if(!EncodeList<Input, Input::Encoder>(buff, inputs)){
      DLOG(ERROR) << "cannot encode transaction inputs: " << inputs.size();
      return false;
    }
    ...
    if(!EncodeList<Output, Output::Encoder>(buff, outputs)){
      DLOG(ERROR) << "cannot encode transaction outputs: " << outputs.size();
      return false;
    }
