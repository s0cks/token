    ...
    friend std::ostream& operator<<(std::ostream& stream, const Status& status){
      switch(status){
        case kFailed:stream << "Failed";
          return stream;
        case kSuccessful:stream << "Success";
          return stream;
        case kCancelled:stream << "Cancelled";
          return stream;
        case kTimedOut:stream << "Timed Out";
          return stream;
    ...
    static void OnTaskFinished(uv_work_t* handle, int status){
      ...
      switch(result.GetStatus()){
        case AsyncTaskResult::kSuccessful:LOG(INFO) << task->GetName() << " finished: " << task->GetResult();
          return;
        case AsyncTaskResult::kFailed:LOG(INFO) << task->GetName() << " failed: " << task->GetResult();
          return;
        case AsyncTaskResult::kCancelled:LOG(WARNING) << task->GetName() << " was cancelled: " << task->GetResult();
          return;
        case AsyncTaskResult::kTimedOut:LOG(WARNING) << task->GetName() << " timed out: " << task->GetResult();
          return;
      }
    }
   public:
